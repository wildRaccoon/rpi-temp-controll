# Інструкція по встановленню на Raspberry Pi

Детальна інструкція по встановленню та налаштуванню системи контролю температури на Raspberry Pi.

## 📋 Передумови

- Raspberry Pi з встановленим Raspberry Pi OS
- Підключення до інтернету
- SSH доступ або прямий доступ до Raspberry Pi

## 🔧 Крок 1: Підготовка системи

### Оновлення системи

```bash
sudo apt update
sudo apt upgrade -y
```

### Встановлення Python та інструментів

```bash
sudo apt install -y python3 python3-pip python3-venv git
```

## 📥 Крок 2: Клонування та налаштування проекту

### Клонування проекту (якщо використовується Git)

```bash
cd ~
git clone <repository-url> tapo
cd tapo
```

Або скопіюйте файли проекту в директорію `~/tapo`

### Створення віртуального середовища

```bash
python3 -m venv venv
source venv/bin/activate
```

### Встановлення залежностей

```bash
pip install --upgrade pip
pip install -r requirements.txt
```

## 🔌 Крок 3: Прошивка Sonoff S60 з Tasmota

### Варіанти прошивки Sonoff S60

Є два основних методи прошивки Tasmota на Sonoff S60:

#### Метод 1: OTA прошивка через eWeLink (рекомендований для початківців)

**Важливо:** Цей метод може не працювати на новіших версіях прошивки Sonoff.

1. Налаштуйте Sonoff S60 через додаток eWeLink
2. Дізнайтесь IP адресу розетки в налаштуваннях роутера
3. Використайте утиліту для OTA прошивки (наприклад, SonOTA)
4. Завантажте прошивку Tasmota для ESP8266

#### Метод 2: Прошивка через UART (надійний метод)

**Необхідне обладнання:**
- USB to TTL адаптер (3.3V!)
- Дроти для підключення
- Паяльник (можливо)

**Кроки:**

1. **Розберіть Sonoff S60:**
   - Відключіть від мережі 220V
   - Відкрутіть гвинти та зніміть кришку
   - Знайдіть контакти GPIO0, TX, RX, VCC, GND

2. **Підключіть USB to TTL адаптер:**
   ```
   USB-TTL     →  Sonoff S60
   ─────────────────────────
   3.3V        →  VCC (3.3V)
   GND         →  GND
   TX          →  RX
   RX          →  TX
   ```
   **ВАЖЛИВО:** Використовуйте тільки 3.3V! 5V може спалити ESP8266!

3. **Введіть режим прошивки:**
   - Замкніть GPIO0 на GND
   - Підключіть USB адаптер до комп'ютера
   - Після підключення можна відпустити GPIO0

4. **Завантажте Tasmota:**
   - Відкрийте [Tasmota Web Installer](https://tasmota.github.io/install/)
   - Або використайте esptool:
   ```bash
   pip install esptool
   esptool.py --port COM3 write_flash -fs 1MB -fm dout 0x0 tasmota.bin
   ```
   - Замініть COM3 на ваш порт (Linux: /dev/ttyUSB0)

5. **Налаштування Tasmota:**
   - Після прошивки Sonoff створить WiFi точку доступу "tasmota-XXXX"
   - Підключіться до неї
   - Налаштуйте WiFi мережу
   - Після підключення відкрийте веб-інтерфейс за IP адресою

6. **Налаштування шаблону для Sonoff S60:**
   ```
   Configuration → Configure Template

   Виберіть "Sonoff S26" або створіть власний:
   - GPIO12: Relay1
   - GPIO13: LED1i (інвертований)
   - GPIO0: Button1
   ```

7. **Налаштування параметрів:**
   - Configuration → Configure Module → Sonoff S26
   - Configuration → Configure MQTT (якщо потрібно)

### Перевірка роботи Tasmota

```bash
# Включити розетку
curl "http://192.168.1.100/cm?cmnd=Power%20On"

# Вимкнути розетку
curl "http://192.168.1.100/cm?cmnd=Power%20Off"

# Статус розетки
curl "http://192.168.1.100/cm?cmnd=Status"

# Toggle (перемикання)
curl "http://192.168.1.100/cm?cmnd=Power%20Toggle"
```

### Корисні команди Tasmota

```bash
# Отримати повну інформацію
curl "http://192.168.1.100/cm?cmnd=Status%200"

# Налаштувати ім'я пристрою
curl "http://192.168.1.100/cm?cmnd=DeviceName%20Heater_Control"

# Налаштувати MQTT (якщо потрібно)
curl "http://192.168.1.100/cm?cmnd=MqttHost%20192.168.1.50"
curl "http://192.168.1.100/cm?cmnd=MqttPort%201883"
curl "http://192.168.1.100/cm?cmnd=Topic%20sonoff_s60"
```

## 🌡️ Крок 4: Підключення датчиків

### Підключення DS18B20 (1-Wire шина)

1. Підключіть всі 3 датчика DS18B20 до GPIO 4 (Pin 7):
   - VCC (червоний) → Pin 1 (3.3V)
   - DATA (жовтий/білий) → Pin 7 (GPIO 4)
   - GND (чорний) → Pin 6 (GND)
   - Підтягуючий резистор 4.7 кОм між DATA та VCC

2. Увімкніть 1-Wire інтерфейс:
   ```bash
   sudo raspi-config
   # Interface Options → 1-Wire → Enable
   ```

3. Перезавантажте:
   ```bash
   sudo reboot
   ```

4. Після перезавантаження перевірте датчики:
   ```bash
   ls /sys/bus/w1/devices/
   # Ви побачите щось на кшталт: 28-00000abc1234
   ```

### Підключення MAX31855 (SPI термопара для димаря)

1. Підключіть MAX31855 до Raspberry Pi через SPI:

   | MAX31855 | Raspberry Pi | GPIO | Пін |
   |----------|--------------|------|-----|
   | **VIN** | 3.3V | - | Pin 1 або 17 |
   | **GND** | Ground | - | Pin 6, 9, 14, 20 (будь-який) |
   | **CLK** | SPI Clock | GPIO 11 | Pin 23 (SCLK) |
   | **CS** | Chip Select | GPIO 8 | Pin 24 (CE0) |
   | **DO** | SPI Data Out | GPIO 9 | Pin 21 (MISO) |

   **Важливо:**
   - MAX31855 працює тільки з **3.3V** (НЕ 5V!)
   - DO (Data Out) на платі = MISO (Master In Slave Out) на Raspberry Pi
   - MAX31855 НЕ використовує MOSI - це read-only пристрій
   - Підключіть термопару типу K до клем + та - на MAX31855

2. Увімкніть SPI інтерфейс:
   ```bash
   sudo raspi-config
   # Interface Options → SPI → Enable → Yes
   ```

3. Перезавантажте Raspberry Pi:
   ```bash
   sudo reboot
   ```

4. Після перезавантаження перевірте, що SPI активовано:
   ```bash
   ls /dev/spi*
   # Має показати: /dev/spidev0.0 та /dev/spidev0.1
   ```

5. Встановіть необхідні бібліотеки для роботи з SPI:
   ```bash
   source venv/bin/activate
   pip install spidev RPi.GPIO
   ```

### Налаштування прав доступу

```bash
sudo usermod -a -G gpio,spi $USER
# Потрібно вийти та увійти знову або перезавантажити
```

## ⚙️ Крок 5: Налаштування конфігурації

### Створення конфігураційного файлу

```bash
cp config.example.yaml config.yaml
nano config.yaml
```

### Визначення ID датчиків DS18B20

```bash
# Після підключення датчиків та увімкнення 1-Wire
ls /sys/bus/w1/devices/

# Ви побачите щось на кшталт:
# 28-011913a8f76d
# 28-011913a78132
# 28-011913c47b0c

# Скопіюйте ці ID та вкажіть в config.yaml для кожного датчика
```

### Налаштування config.yaml

Відредагуйте `config.yaml`:

1. **Датчики DS18B20:**
   ```yaml
   sensors:
     ds18b20:
       boiler:
         enabled: true
         device_id: "28-xxxxx"  # Замініть на реальний ID
         name: "Котел"
       accumulator_bottom:
         enabled: true
         device_id: "28-yyyyy"  # Замініть на реальний ID
         name: "Термоакумулятор (низ)"
       accumulator_top:
         enabled: true
         device_id: "28-zzzzz"  # Замініть на реальний ID
         name: "Термоакумулятор (верх)"
   ```

2. **Датчик MAX31855 (термопара димаря):**
   ```yaml
   sensors:
     max31855:
       chimney:
         enabled: true
         cs_pin: 8              # GPIO 8 (Pin 24, CE0)
         spi_port: 0            # SPI0
         spi_device: 0          # SPI Device 0
         name: "Димар"
   ```

   **Примітка:** Якщо потрібно використати інший CS пін (не GPIO 8),
   змініть значення `cs_pin` та підключіть CS дріт до відповідного GPIO.

3. **Розетка Sonoff S60 з Tasmota:**
   ```yaml
   sonoff:
     ip_address: "192.168.1.100"  # IP адреса розетки Sonoff S60
     # Tasmota використовує HTTP API або MQTT
     # Для HTTP API потрібна тільки IP адреса
     # Для MQTT налаштуйте брокер нижче (опціонально)

   mqtt:  # Опціонально, якщо використовуєте MQTT
     enabled: false
     broker: "192.168.1.50"
     port: 1883
     username: ""  # Опціонально
     password: ""  # Опціонально
     topic_prefix: "sonoff_s60"
   ```

4. **Порогові температури:**
   ```yaml
   control:
     boiler_critical_temp: 85.0
     accumulator_critical_temp: 80.0
     boiler_safe_temp: 70.0
     accumulator_safe_temp: 65.0
     chimney_low_temp: 100.0
   ```

## 🧪 Крок 6: Тестування

### Тестовий запуск

```bash
cd ~/tapo
source venv/bin/activate
python main.py --test-mode --scenario normal
```

Відкрийте браузер: `http://raspberry-pi-ip:8080/status.html`

### Тест з реальними датчиками

```bash
python main.py
```

Перевірте логи:
```bash
tail -f logs/temperature.log
```

## 🚀 Крок 7: Налаштування як systemd сервіс

### Створення сервіс файлу

```bash
cd ~/tapo
sudo cp tapo-temperature.service /etc/systemd/system/
```

### Редагування шляхів (якщо потрібно)

Якщо проект знаходиться не в `/home/pi/tapo`, відредагуйте файл:

```bash
sudo nano /etc/systemd/system/tapo-temperature.service
```

Змініть шляхи:
- `WorkingDirectory` - шлях до проекту
- `ExecStart` - шлях до Python та main.py
- `User` - користувач (за замовчуванням `pi`)

### Активація та запуск сервісу

```bash
# Перезавантажити systemd
sudo systemctl daemon-reload

# Увімкнути автозапуск
sudo systemctl enable tapo-temperature.service

# Запустити сервіс
sudo systemctl start tapo-temperature.service

# Перевірити статус
sudo systemctl status tapo-temperature.service
```

### Управління сервісом

```bash
# Переглянути статус
sudo systemctl status tapo-temperature.service

# Переглянути логи
sudo journalctl -u tapo-temperature.service -f

# Перезапустити
sudo systemctl restart tapo-temperature.service

# Зупинити
sudo systemctl stop tapo-temperature.service

# Вимкнути автозапуск
sudo systemctl disable tapo-temperature.service
```

## 🔍 Крок 8: Перевірка роботи

### Перевірка через веб-інтерфейс

Відкрийте в браузері:
- `http://raspberry-pi-ip:8080/status.html` - поточний стан
- `http://raspberry-pi-ip:8080/charts.html` - графіки

### Перевірка через API

```bash
# Статус системи
curl http://localhost:8080/api/status

# Датчики
curl http://localhost:8080/api/sensors

# Стан системи
curl http://localhost:8080/api/system
```

### Перевірка логів

```bash
# Логи програми
tail -f logs/temperature.log

# Логи systemd
sudo journalctl -u tapo-temperature.service -f
```

## 🛠️ Крок 9: Налаштування firewall (опціонально)

Якщо потрібен доступ з інших пристроїв:

```bash
sudo ufw allow 8080/tcp
```

## 📊 Крок 10: Моніторинг та обслуговування

### Перевірка бази даних

```bash
cd ~/tapo
source venv/bin/activate
sqlite3 data/temperature.db "SELECT COUNT(*) FROM temperature_readings;"
```

### Резервне копіювання

```bash
# Створити резервну копію бази даних
cp data/temperature.db data/temperature.db.backup.$(date +%Y%m%d)
```

### Оновлення програми

```bash
cd ~/tapo
source venv/bin/activate
git pull  # Якщо використовується Git
pip install -r requirements.txt --upgrade
sudo systemctl restart tapo-temperature.service
```

## ❗ Вирішення проблем

### Датчики не знаходяться

1. Перевірте підключення:
   ```bash
   ls /sys/bus/w1/devices/
   ```

2. Перевірте, чи увімкнено 1-Wire:
   ```bash
   sudo raspi-config
   ```

3. Перевірте логи:
   ```bash
   tail -f logs/temperature.log
   ```

### Розетка Sonoff S60 з Tasmota не підключається

1. Перевірте IP адресу та мережеве підключення:
   ```bash
   ping 192.168.1.100
   ```

2. Перевірте доступність веб-інтерфейсу Tasmota:
   ```bash
   curl http://192.168.1.100/cm?cmnd=Status
   ```

3. Перевірте, що Tasmota правильно налаштована на Sonoff S60

4. Якщо використовуєте MQTT, перевірте з'єднання з брокером

### Сервіс не запускається

1. Перевірте логи:
   ```bash
   sudo journalctl -u tapo-temperature.service -n 50
   ```

2. Перевірте шляхи в сервіс файлі
3. Перевірте права доступу:
   ```bash
   ls -la ~/tapo
   ```

## 📞 Підтримка

Для детальної інформації дивіться:
- [README.md](README.md) - загальна інформація
- [PLAN.md](PLAN.md) - детальний план проекту

